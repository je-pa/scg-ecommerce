package com.scgecommerce.global.security.jwt.util;
...
import com.scgecommerce.global.util.exception.ExceptionCode;
...
public class TokenProvider {
  ...
  public Mono<Authentication> getAuthentication(String token) {
    ...
    return this.userDetailsService.findByUsername(myEncoder.decrypt(this.getUsername(token)))
        .map(userDetails -> {
  ...
  private Date getExpiration(String token) {
    if (!StringUtils.hasText(token)) {
      throw CustomException.from(ExceptionCode.BAD_PADDING.EXPIRED_JWT_TOKEN);
    }
